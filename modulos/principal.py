...
class Iniciador(tk.Tk, object):
    ...
    def productes(self):
        ...
        if self.lateral_derecho is None:
            ...
            self.labels_entry(text="Nom", x=230, y=50)
            self.labels_entry(text="Us", x=400, y=50)
            self.labels_entry(text="Descripció", x=570, y=50)
            self.entrys(x=90, y=50, ancho=20)
            self.entrys(x=270, y=50, ancho=20)
            self.entrys(x=430, y=50, ancho=20)
            self.entrys(x=660, y=50, ancho=100)
            self.botones_form()
    ...
    def cassos(self):
        if (self.bcasos['state'] == tk.NORMAL):
            ...
            if self.lateral_derecho is None:
                ...
                self.labels_entry(text="Descripció", x=460, y=50)
                self.entrys(x=100, y=50, ancho=20)
                self.entrys(x=320, y=50, ancho=20)
                self.entrys(x=550, y=50, ancho=120)
                self.botones_form()
